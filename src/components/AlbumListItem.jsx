...
const AlbumListItem = ({ albumInfo, index, song, media, favourites, playPause, setPlaying, favouriteTrack, unFavouriteTrack }) => {
    ...
    const playHandler = () => {
        ...
        if (!song.preview_url) {
            ...
            setPlaying({ ...song, album: { images: [...albumInfo.images], name: albumInfo.name } });
    ...
    return (
        <div key={index} className="row mx-0 d-flex align-items-center playlist-item-name p-2">
            ...
            <div className="col-3 col-md-2 d-flex align-items-center">
                {favourites.tracks.find(item => item?.id === song?.id) ? <AiFillHeart className="me-2" style={{ fontSize: "20px", color: "1db954" }} onClick={() => { unFavouriteTrack({ ...song, album: { images: [...albumInfo.images] , name: albumInfo.name } }); }} />
                    : <AiOutlineHeart className="me-2" style={{ fontSize: "20px" }} onClick={() => { favouriteTrack({ ...song, album: { images: [...albumInfo.images], name: albumInfo.name } }); }} />}
                <span>
