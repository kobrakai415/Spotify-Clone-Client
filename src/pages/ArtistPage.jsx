...
import { addArtistToFavourites, removeArtistFromFavourites, playPause, addTrackToFavourites, removeTrackFromFavourites, setCurrentSong, setQueue, fetchTopTracks } from "../redux/actions"
...
const mapDispatchToProps = (dispatch) => ({
    ...
    fetchTopTracks: (id, token) => dispatch(fetchTopTracks(id, token))

})

const ArtistPage = ({ token, media, data, favourites, playPause, favouriteArtist, unFavouriteArtist, setPlaying, setQueue, fetchTopTracks }) => {
    ...
    const playHandler = () => {
        const songToPlay = topTracks.find(item => item.preview_url !== null)
                    ...
                    <Col className="p-3 p-md-5 mt-5">
                        ...
                        <Row className="pb-3">
                            <Col xs={12}>
                                ...
                                {media.play && topTracks.some(item => item.id === media.selectedSong.id) &&
                                    <BiPause onClick={playPause} className="me-4" style={{ fontSize: "55px", backgroundColor: "1db954", borderRadius: "50%" }} />}

                                {media.play && !topTracks.some(item => item.id === media.selectedSong.id) &&
                                    <CgPlayButton onClick={playPause} className="me-4" style={{ fontSize: "55px", backgroundColor: "1db954", borderRadius: "50%" }} />}
